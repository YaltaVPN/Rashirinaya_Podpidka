...
# СТАРАЯ НАДЕЖНАЯ РЕГУЛЯРКА: Ищет конфиги в любом месте текста/HTML
PROXY_REGEX = re.compile(r'(?:vless|vmess|ss|trojan|tuic|hysteria2|hy2)://[^\s"\'`,<>\]\}]+')

# --- СЛОВАРИ И БАЗЫ ДАННЫХ ИЗ ВАШЕГО КОДА ---
FLAG_TO_CODE = {
    "🇦🇫": "AF", "🇦🇱": "AL", "🇩🇿": "DZ", "🇦🇩": "AD", "🇦🇴": "AO",
    "🇦🇷": "AR", "🇦🇲": "AM", "🇦🇺": "AU", "🇦🇹": "AT", "🇦🇿": "AZ",
    "🇧🇩": "BD", "🇧🇾": "BY", "🇧🇪": "BE", "🇧🇷": "BR", "🇧🇬": "BG",
...
def parse_config_line(line):
    """Парсит и валидирует ОДНУ ссылку, найденную регулярным выражением"""
    line = line.strip()
    if not line:
        return None
    ...
    if not any(url_part.startswith(p) for p in ('vless://', 'vmess://', 'trojan://', 'ss://', 'tuic://', 'hysteria2://', 'hy2://')):
        return None
...
def fetch_source(url):
    """Скачивает контент и вытаскивает все прокси старым надежным способом (регуляркой)"""
    try:
        ...
        # Автодетекция Base64 (если вся страница зашифрована)
        try:
        ...
        # ВОЗВРАЩЕН СТАРЫЙ МЕТОД: Ищем регуляркой во всем тексте, игнорируя HTML-теги и структуры строк
        found_links = PROXY_REGEX.findall(text)
        
        configs = []
        for link in found_links:
            parsed = parse_config_line(link)
            if parsed:
...
def log_to_sheet(total, cidr_count, unique_sni):
    ...
    # Держим размер файла логов в пределах 1000 строк
...
def main():
    ...
    print("📡 Начало параллельной загрузки источников старым методом регулярных выражений...")
    ...
    # --- ЖЕСТКИЙ КОНТРОЛЬ ТЕКСТОВОГО ЛИМИТА В 95.00 МБ ---
    ...
    for cfg in all_configs:
        ...
        if current_size_bytes + line_size > max_bytes:
            print(f"\n⚠️ Предупреждение: Достигнут лимит размера текстового файла в {MAX_MB} МБ.")
    ...
    # Сохраняем чистый plain-text
    save_to_drive(final_content, OUTPUT_FILE)
    print(f"✅ Файл подписки '{OUTPUT_FILE}' успешно перезаписан.")
    print(f"📦 Итоговый вес файла подписки: {os.path.getsize(os.path.join(BASE_DIR, OUTPUT_FILE)) / (1024*1024):.2f} МБ")
